...
('parents', 'last_login_ip', '마지막 로그인 IP. 자격 증명을 실제로 확인한 때만 갱신되고 토큰 재발급으로는 바뀌지 않는다'),
('parents', 'last_login_at', '마지막 접속 시각. 로그인과 리프레시 토큰 재발급이 모두 갱신한다. 한 번도 접속한 적 없거나 이 컬럼이 생기기 전이면 비어 있다'),
