...
**푸시를 아직 안 쓴다면 이 두 변수를 아예 넣지 않는다.** 자리를 채우려고 아무 값이나
넣으면 앱이 기동하지 않는다. 비어 있으면 로그만 남기는 발송기로 바뀌어 정상으로 뜨고,
답변 알림은 DB에 쌓여서 사용자가 앱 안에서 확인할 수 있다. 값이 잘못됐을 때 그냥
뜨게 두지 않는 것은 일부러 그렇게 한 것이다 - 답변은 등록되는데 푸시만 조용히 안
나가는 상태가 제일 나쁘다.
...
| `FCM_CREDENTIALS 이 자격증명으로 보이지 않습니다` | 자리만 채운 값을 넣었다. 푸시를 안 쓰면 지운다 |
